    ...
    if (!/^[a-zA-Z0-9_\-]+$/.test(approval.openclawApprovalId)) {
      ...
      try {
        ...
        await ssh.execCommand(
          `curl -s -X POST http://127.0.0.1:${instance.mcPort}/resolve-approval -H 'Content-Type: application/json' -d '{"openclawApprovalId":"${approval.openclawApprovalId}","resolution":"deny"}'`
        );
        ssh.dispose();
      } catch (err) {
        console.error("Failed to forward denial to OpenClaw:", err);
      }
    }
  }
