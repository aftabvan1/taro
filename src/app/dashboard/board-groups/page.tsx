...
export default function BoardGroupsPage() {
  ...
  const [groups, setGroups] = useState<MCBoardGroup[]>([]);
  ...
  const fetchGroups = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/mission-control/board-groups", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error("Failed to fetch board groups");
  ...
  async function handleCreateGroup() {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to create board group");
  ...
  async function handleDeleteGroup(id: string) {
    ...
    try {
      const res = await fetch(`/api/mission-control/board-groups/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error("Failed to delete board group");
      ...
      >
        {groups.map((group) => (
          <motion.div
            ...
            className="group cursor-pointer overflow-hidden rounded-lg border border-white/[0.06] bg-[#0c0c0d] transition-all hover:border-emerald-500/15 hover:shadow-[0_0_20px_rgba(16,185,129,0.04)]"
          >
            ...
            <div className="p-4">
              <div className="flex items-start justify-between">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-1.5">
                      ...
                      <span className="font-mono text-[10px] text-zinc-500">
                        {group.board_count}{" "}
                        {group.board_count === 1 ? "board" : "boards"}
                      </span>
                    </div>
                  </div>
                </div>
                <button
                  ...
                  disabled={deleting.has(group.id)}
        ...
        {showNewGroup ? (
          ...
          >
            ...
            <span className="font-mono text-xs text-zinc-600">NEW GROUP</span>
          </motion.button>
        )}
      </motion.div>
      ...
      {groups.length === 0 && !showNewGroup && (
        <div className="flex flex-col items-center justify-center gap-4 rounded-lg border border-white/[0.06] bg-[#0c0c0d] py-16">
          ...
          <p className="font-mono text-sm text-zinc-400">
            No board groups created
          </p>
          <p className="max-w-sm text-center font-mono text-xs text-zinc-600">
            Board groups let you organize related boards together for easier
            navigation.
          </p>
        </div>
      )}
